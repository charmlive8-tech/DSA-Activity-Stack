...
def infix_to_postfix(expression, trace=None):
    ...
    while not operators.is_empty():
        top = operators.pop()
        if top == "(":
            raise ValueError("Unbalanced parenthese: no matching )")
        output.append(top)
    if trace is not None:
...
def evaluate_postfix(expression, trace=None):
    ...
    for token in tokenize(expression):
        if token in PRECEDENCE:
            if values.size() < 2:
                raise ValueError(f"not enough operands" + token)
            right = values.pop()
            left = values.pop()
        ...
        if trace is not None:
                trace.append((token, ' '.join(_fmt(v)for v in values_.items)))
    if values.size() != 1:
